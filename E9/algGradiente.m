function [q_f, iter, deltaQ, currentError] = algGradiente(condIniz, riferimento)
    % iperparametri
    ...
    while(iter < maxIter && deltaQ > minQ && currentError > minE)
        ...
        q_1 = q(1);
        q_2 = q(2);
        JT = [-sin(q_1)-sin(q_1+q_2), cos(q_1)+cos(q_1+q_2); -sin(q_1+q_2), cos(q_1+q_2)];
        fr = [cos(q_1)+cos(q_1+q_2); +sin(q_1)+sin(q_1+q_2)];
        newQ = q + alpha*JT*(rRif-fr);
        q_f = [q_f, newQ];
        deltaQ = norm(newQ-q);
